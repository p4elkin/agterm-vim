...
struct NormalModeStateTests {
    ...
    @Test func startsInactiveAndPassesKeysThrough() {
        var mode = NormalModeState(binds: [NormalModeBind(keybind: [d], action: .toggleSplit)])
    ...
    @Test func singleKeyFires() {
        var mode = state([NormalModeBind(keybind: [d], action: .toggleSplit)])
        #expect(mode.advance(d) == .fired(.toggleSplit))
        #expect(mode.isActive)
    }

    @Test func sequenceFires() {
        var mode = state([NormalModeBind(keybind: [space, s], action: .toggleSidebar)])
    ...
    @Test func exitKeyBindsAsSequenceTail() {
        var mode = state([NormalModeBind(keybind: [space, i], action: .toggleSidebar)])
        #expect(mode.advance(space) == .armed)
    ...
    @Test func modifiedExitKeyDoesNotLeave() {
        var mode = state([NormalModeBind(keybind: [ctrlI], action: .toggleSplit)])
