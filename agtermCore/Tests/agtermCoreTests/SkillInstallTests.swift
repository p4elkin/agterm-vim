...
struct SkillInstallTests {
    ...
    @Test func bundledSkillDocumentsEventSubscriptionCommand() throws {
        ...
        #expect(skill.contains("park [on|off|toggle]"))
        #expect(reference.contains("`session park [on|off|toggle] [--target] [--window W]`"))
        #expect(examples.contains("agtermctl session park on"))
    ...
    @Test func bundledSkillDocumentsSessionRecency() throws {
        ...
        #expect(reference.contains("fourteen top-level read-only fields"))
        #expect(reference.contains("All fourteen are read-only projections"))
        #expect(skill.contains("fourteen read-only top-level fields"))

        let fields = treeTopLevelFieldNames
        #expect(fields.count == 14, "ControlTree's top-level fields changed; update the skill's counts")
        for field in fields {
