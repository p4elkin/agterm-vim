...
public enum AgentHooksInstall {
    ...
    private static func migrateClaudeEntriesToAdapter(_ hooks: inout [String: Any], scriptDir: String) -> Bool {
        ...
        for hook in claudeHooks {
            ...
            // every state this installer EVER generated for the event, mapped onto the state to write now.
            // UserPromptSubmit predates the --blink promotion (a9e678d9) and an install from that window —
            // source builds only, no tag carries the bare form — still holds `active`; every other event has a
            // single historical form. A stale state migrates onto the adapter AND the current state in one
            // rewrite, so the fork's second `mark` command is left for the merge below to append.
            var migrations = Dictionary(uniqueKeysWithValues: hook.states.map { ($0, $0) })
            if hook.event == "UserPromptSubmit", let current = hook.states.first {
                migrations["active"] = current
            }
            var eventChanged = false
            for index in entries.indices {
                guard var commands = entries[index]["hooks"] as? [[String: Any]] else { continue }
                var entryChanged = false
                for slot in commands.indices {
                    guard let command = commands[slot]["command"] as? String, command.hasPrefix(generated),
                          let state = migrations[String(command.dropFirst(generated.count))] else { continue }
                    commands[slot]["command"] = replacement + state
                    entryChanged = true
                }
                if entryChanged {
                    entries[index]["hooks"] = commands
