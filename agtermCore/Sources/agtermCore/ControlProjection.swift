...
public struct ControlSessionNode: Codable, Sendable, Equatable {
    ...
    /// lived in.
    public let turn: Int?
    /// How many turns of this session are bookmarked (fork only); nil/omitted when none. The read side of
    /// `session.bookmark.add`/`.remove`; persisted with the bookmarks, unlike `turn`.
