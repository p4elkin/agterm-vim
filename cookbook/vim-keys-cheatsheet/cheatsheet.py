...
Normal-mode binds (`nmap`) are read too, and they get a section of their own. They are bare keys
that fire nothing until the mode is on, so a row for one among the chords would read as a key to
press in a terminal, where it types a letter. Their rows are written from the keymap rather than
drafted, and they are keyed apart everywhere, because `nmap s` and a bare `map s` spell the same.
...
NORMAL_HEADING = "## Normal mode"
NORMAL_INTRO = ("Bare keys, live only while normal mode is on. Press {entry} to enter the mode,"
                " `i` or Esc to leave it.")
NORMAL_NO_ENTRY = ("Bare keys, live only while normal mode is on. Nothing in `keymap.conf` maps a"
                   " chord to `normal_mode`, so none of them can be reached until one does.")
...
# keeps whichever was seen first. No agterm chord can contain a colon, so the prefix is free.
NORMAL = "normal:"


def normal_key(chord):
    return NORMAL + chord
...
def bare(key):
    """The chord itself, whichever namespace the key came from."""
    return key[len(NORMAL):] if is_normal(key) else key


def named(key):
    """A key as a banner names it. A bare key means nothing without the mode it lives in."""
    return f"`{bare(key)}` in normal mode" if is_normal(key) else f"`{key}`"
...
def normal_bindings(keymap):
    """Key and label for every `nmap` bind, keyed apart from the chords above.
...
def normal_entry(keymap):
    """The chord that arms normal mode, or None, in which case no `nmap` bind can be reached."""
    for chord, action in MAP_LINE.findall(keymap):
...
def split_rows(sheet):
    """The sheet's table rows, split into the normal-mode section and everything else.

    Which table a row sits in is the only thing that tells `j` the bare key from `j` the chord, so
    the two are searched separately rather than in one pile.
    """
    inside, outside, in_normal = [], [], False
    for line in sheet.splitlines():
        stripped = line.lstrip()
        if stripped.startswith("## "):
            in_normal = stripped.rstrip() == NORMAL_HEADING
        if stripped.startswith("|"):
            (inside if in_normal else outside).append(stripped)
...
def searchable(sheet):
    ...
    """
    inside, outside = split_rows(sheet)
    return "\n".join(outside) if inside or outside else sheet


def normal_searchable(sheet):
    """The rows a normal-mode bind has to be found in: its own section's table, and only that."""
    return "\n".join(split_rows(sheet)[0])
...
def undocumented_normal(pairs, sheet):
    """Normal-mode binds with no row in the normal-mode section."""
    flat = normal_searchable(sheet)
    return sorted({key for key, _ in pairs if not documented(bare(key), flat)})
...
def binding_lines(keymap):
    """Key -> what its keymap line does, normalised. Normal-mode binds are keyed apart."""
    lines = {}
    for line in keymap.splitlines():
        ...
        # `nmap` lines belong here for the same reason the others do: a bind that keeps its key
        # and changes its target leaves a row naming the right key and describing the wrong thing,
        # and nothing else in the pass would notice.
        nmap = NMAP_LINE.match(stripped)
        if nmap:
            lines[normal_key(nmap.group(1))] = normalise(stripped)
    return lines
...
def rebound(keymap, sheet):
    ...
    if sheet_is_current():
        return []
    flat, normal_flat = searchable(sheet), normal_searchable(sheet)
    return sorted(key for key, line in binding_lines(keymap).items()
                  if key in before and before[key] != line
                  and documented(bare(key), normal_flat if is_normal(key) else flat))
...
def insert_section(sheet, block):
    """Add a whole section, ahead of the holding section when the sheet has one.

    The order is not cosmetic. Drafted rows are appended at the end of the file, and the end of
    the file is the holding section's table only while nothing sits after it.
    """
    body = sheet if sheet.endswith("\n") else sheet + "\n"
    if DRAFT_HEADING in body:
        head, _, tail = body.partition(DRAFT_HEADING)
        return head.rstrip("\n") + "\n\n" + block + "\n" + DRAFT_HEADING + tail
    return body.rstrip("\n") + "\n\n" + block


def add_normal(sheet, entry, pairs):
    """Write these normal-mode binds into a section of their own, creating it when it is absent.

    Never into the chord tables, and that is the whole reason this section exists. A row saying
    `e` among the chords reads as a key to press, and pressing it in a terminal types a letter:
    it fires a bind only while the mode is on. The section's opening line is what says how to
    turn the mode on, so the keys under it are reachable at all.

    Rows are mechanical here — the key and its target — where the chord tables get a drafted
    sentence. The model answers with a chord string and nothing else, so a bare `j` in its reply
    could belong to either table, and a row filed under the wrong one is worse than a plain one.
    Reword them like the rest of the sheet; nothing rewrites them afterwards.
    """
    if not pairs:
        return sheet
    rows = "".join(f"| {chord_md(bare(key))} | {cell(label)} |\n" for key, label in pairs)
    lines = sheet.splitlines(keepends=True)
    start = next((i for i, line in enumerate(lines) if line.strip() == NORMAL_HEADING), None)
    if start is None:
        intro = NORMAL_INTRO.format(entry=chord_md(entry)) if entry else NORMAL_NO_ENTRY
        head = f"{NORMAL_HEADING}\n\n{intro}\n\n| key | does |\n|---|---|\n"
        return insert_section(sheet, head + rows)
    ...
    # Appended after the section's last row, not after the file: the holding section usually sits
    # below this one. The heading kept with its table deleted gets a header back rather than
    # loose rows.
    at, header = (end, "") if end > start else (last, "\n| key | does |\n|---|---|\n")
    if not lines[at].endswith("\n"):
        lines[at] += "\n"
    return "".join(lines[:at + 1]) + header + rows + "".join(lines[at + 1:])
...
def first_sheet(keymap, pairs):
    ...
    normal, entry = normal_bindings(keymap), normal_entry(keymap)
    if not DRAFT or not chords:
        return add_normal(starter(pairs), entry, normal), {}
    hud = Hud()
    try:
    ...
    return add_normal(generated(rows), entry, normal), {row["chord"]: row["does"] for row in rows}
...
def sync(keymap):
    ...
    normal = normal_bindings(keymap)
    missing = undocumented(pairs, sheet)
    fresh = undocumented_normal(normal, sheet)
    ...
    # A normal-mode row is never the model's, so it is never restated: it is named in the banner
    # and left for the reader, the same as a row they wrote themselves.
    ours = [chord for chord in stale if not is_normal(chord) and owns(sheet, chord)]
    theirs = [chord for chord in stale if chord not in ours]

    # No model call, so this happens whatever the drafting settings are.
    if fresh:
        wanted = set(fresh)
        sheet = add_normal(sheet, normal_entry(keymap),
                           [pair for pair in normal if pair[0] in wanted])
        write(SHEET, sheet)

    if (not missing and not stale) or not DRAFT:
        write(STAMP, mtime)
        # The rows just written say what those lines do now, which is the baseline a later rebind
        # of one of them is measured against. A stale chord keeps its old line, as everywhere.
        if fresh:
...
def render(keymap):
    ...
    normal = normal_bindings(keymap)
    if not SHEET.exists():
        write(SHEET, add_normal(starter(pairs), normal_entry(keymap), normal))
        print(f"> **Generated `{SHEET}` from your keymap.**")
        print(">")
        print("> Rewrite it in your own words. This run shows what was written.")
        print()

    # The section names the entry chord as it stood when the section was written, so an unbound
    # mode is said here instead, where it is read from the keymap on every press.
    if normal and not normal_entry(keymap):
        print("> **Nothing arms normal mode, so no `nmap` bind can be reached.**")
        print(">")
        print("> Add a `map <chord> normal_mode` line — `map ctrl+space normal_mode` is the usual"
              " one — and reload the keymap.")
        print()

    sheet = SHEET.read_text(encoding="utf-8")
    missing = undocumented(pairs, sheet) + undocumented_normal(normal, sheet)
    if missing:
        print(f"> **This sheet has drifted from `{KEYMAP.name}`.**")
        print(">")
        print("> Bound but not documented: " + ", ".join(named(c) for c in missing))
        print()
