...
- **The reply prompt takes a screen of its own.** fzf runs full-screen, so its `execute` hands the reply script the normal screen, which still holds whatever the pane was showing before the viewer opened. A prompt painted there reads as the chat having vanished behind an old build log. The reply switches to the alternate screen, names the room it is answering and shows the last message in it, then gives the screen back on every exit path. Escape cancels alongside an empty line, because `read` receives Escape as a raw byte and would otherwise write it into the room as a message.
- **The message preview moves underneath in a narrow pane.** Side by side needs width for both halves at once, and a right pane is often around 48 columns, where `right,60%` leaves the list 19 and the message 29. At that size the chat reads as missing rather than cramped. Under 100 columns the preview goes below the list instead, so the message gets the pane's whole width.
